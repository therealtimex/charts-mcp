...
export class ArcDiagramBuilder extends ChartBuilder {
  ...
  buildHtml(spec: ChartSpec): string {
    ...
    // Normalize theme: treat 'default' as 'classic' for G2 compatibility
    const themeInput = (spec.theme as any) || 'classic';
    const theme = themeInput === 'default' ? 'classic' : themeInput;
  ...
  function computeData(raw) {
    ...
    if ('${layout}' === 'circular') {
      ...
      links.forEach((edge) => {
        ...
        if (sourceNode && targetNode) {
          ...
          for (let i = 0; i <= steps; i++) {
            ...
            arcData.push({ x, y, value, linkId: String(edge.source) + '-' + String(edge.target), sourceName: sourceNode.label || sourceNode.name || sourceNode.id, targetName: targetNode.label || targetNode.name || targetNode.id, type: 'link' });
          }
        }
      });
      ...
      nodes.forEach((node, i) => {
        ...
        nodeData.push({ ...node, x, y: nodeY, type: 'node' });
      });
      links.forEach((edge) => {
  ...
  function draw(processed) {
    const linkData = processed.filter((d) => d.type === 'link');
    const nodeData = processed.filter((d) => d.type === 'node');
  ...
  const isFetch = ${JSON.stringify(typeof (data as any)?.type === 'string' && (data as any)?.type === 'fetch' && (data as any)?.value)};
  if (isFetch) {
    fetch(${JSON.stringify((data as any)?.value || '')})
      .then((r) => r.json())
      .then((raw) => draw(computeData(raw)))
      .catch((e) => console.error('Failed to load data for arc-diagram:', e));
  } else {
    const raw = ${JSON.stringify(data)};
    draw(computeData(raw));
  }
`;
